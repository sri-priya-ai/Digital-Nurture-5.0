# Python Backend Frameworks — Hands-On 1 to 10

The Course Management API built progressively across Django (1–3), Flask (4–5), and FastAPI (6–9), with a Flask microservices split for Hands-On 10. Hands-On 7, 8, and 9 continue the same FastAPI app built in Hands-On 6.
...
Same app, refactored: `/api/v1/` versioning, PATCH, Location headers, pagination, search, standardised error format.
...
Three separate Flask apps, each in its own subfolder, each with its own `requirements.txt`. Run each in its own terminal:
...
Stop `course_service` and repeat the enroll call to see the gateway return `503 Service Unavailable`.
